...
const avatarModules = import.meta.glob('../../assets/imgs/avatars/*.svg');
...
function updateLanguage(value) {
  selectOptions.forEach((option) => {
    if (option.value === value) {
      option.selected = true;
    } else if (option.selected === true && option.value !== value) {
...
<template>
  <div class="player-planel">
    <div class="player-planel__head">
      <Select :options="selectOptions" @change="updateLanguage($event)">
        <svg
    ...
    >
      <Avatar type="choice" :avatar="avatarSelected" />
      <Field
