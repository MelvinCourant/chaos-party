...
provide('dotsMenuActions', hostActions);
...
async function giveHost(playerId) {
  await fetch(`${env.VITE_URL}/api/users/update-host`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      'Accept-Language': userStore.language,
...
<template>
  <main class="lobby">
    ...
    <div class="lobby__container">
      <Settings />
      <Players :players="players" @actionClicked="handleActions" />
      <div class="party">
        <Modes :modes="modes" @selectMode="updateMode($event)" />
